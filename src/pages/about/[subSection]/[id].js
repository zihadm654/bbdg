...
const SubSection = (props) => {
    ...
    return (
        ...
        <CustomHead title={Data.heading} />
            <Banner>
                <p className={BannerStyle.smallHeading}>about <i className='fas fa-chevron-right'></i> {Data.parent.split("-").join(" ")} <i className='fas fa-chevron-right'></i><span className={BannerStyle.lowOpacity}> {Data.heading}</span></p>
                <br></br>
...
export async function getServerSideProps({ params: { subSection, id } }) {
    const response = await axios.get(`${BaseApi}/about/${id}`)
